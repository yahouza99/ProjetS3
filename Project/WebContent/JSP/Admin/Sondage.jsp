                ...
                <div class="row justify-content-center ">
                  <!-- col -->
                   <div class="col-lg-6 text-center ">
                    <a href="/Project/ServletSondage?action=allsondage" ><h5 class=" mx-5" style="color: #594E83;" ><i class="fa-solid fa-film"></i> Films & Series</h5></a>
                  </div>
                  <div class="col-lg-6 text-center ">
                   <a href="/Project/ServletSondage?action=allsondage_t"> <h5 class=" mx-5" ><i class="fa-solid fa-music"></i> Titres</h5></a>
                  </div>
                  <!-- col end -->
                </div>
                <!-- row end -->
                ...
                <div class="row adm-row">
                    <table class="table">
                        ...
                        <c:forEach var="sondage" items="${listsondages}"  varStatus="boucle">
                          <tr>
                                    <td>
                                        <c:out value="${sondage.getDate_sondage()}" />
                                    </td>
                                    <td>
                                     <c:forEach var="movie" items="${listmovies}"  varStatus="boucle">
                                        <c:if test="${sondage.getId1()==movie.getIdmovie()}">
                                        <c:out value="${movie.getNom_movie()}" />
                                        </c:if>
                                      </c:forEach>
                                    </td>
                                    <td>
                                       <c:forEach var="movie2" items="${listmovies}"  varStatus="boucle2">
                                        <c:if test="${sondage.getId2()==movie2.getIdmovie()}">
                                        <c:out value="${movie2.getNom_movie()}" />
                                        </c:if>
                                      </c:forEach>
                                    </td>
                                    <td>
                                      <c:if test="${sondage.getSatut_sondage()==0}">
                                         <a class="text-danger" href="/Project/ServletSondage?action=updatesondage&id=<c:out value='${sondage.getIdsondage()}' />&statu=1"><i class="fa-solid fa-circle-stop"></i></a>
                                      </c:if>
                                      <c:if test="${sondage.getSatut_sondage()==1}">
                                         <a class="text-success" href="/Project/ServletSondage?action=updatesondage&id=<c:out value='${sondage.getIdsondage()}' />&statu=0"><i class="fa-solid fa-check"></i></a>
                                      </c:if>
                                    </td>
                                    
                                    <td><a class="text-info" href="/Project/ServletSondage?action=findsondage&id=<c:out value='${sondage.getIdsondage()}'/>">
                                       <i class="fa-solid fa-pen"></i></a> 
                                       |<a class="text-danger" href="/Project/ServletSondage?action=deletesondage&id=<c:out value='${sondage.getIdsondage()}'/>">
